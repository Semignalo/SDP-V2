...
import { Edit2, Search, UserCheck, UserX, Shield, Crown, User as UserIcon } from 'lucide-react';
...
export default function Users() {
    ...
    const handleEditRole = async (user) => {
        const { value: newRole } = await Swal.fire({
            ...
            confirmButtonColor: '#0f172a', // Tailwind slate-900 (primary)
        ...
        if (newRole && newRole !== user.role) {
            try {
                ...
                // Force tier diamomd if starcenter
                const newTier = newRole === 'starcenter' ? 'diamond' : user.tier;

                await updateDoc(userRef, { role: newRole, tier: newTier });
                
                // Update local state
                setUsers(users.map(u => u.id === user.id ? { ...u, role: newRole, tier: newTier } : u));
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {filteredUsers.map((user) => {
                                    const dateStr = user.createdAt?.toDate ? user.createdAt.toDate().toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric' }) : '-';
                                    return (
                                        <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                            <td className="p-4 text-sm font-medium text-gray-900">
                                                {user.name || '-'}
                                                <div className="text-xs text-gray-400 font-normal mt-0.5">ID: {user.id.slice(0, 8)}...</div>
                                            </td>
                                            ...
                                            <td className="p-4">
                                                <div className="flex flex-col gap-2 items-start">
                                                    {getRoleBadge(user)}
                                                    {user.role !== 'admin' && getTierBadge(user.tier)}
                                                </div>
                                            </td>
                                            <td className="p-4 text-sm font-bold text-gray-900">
                                                Rp. {(user.cumulativeSpending || 0).toLocaleString('id-ID')}
                                            </td>
                                            <td className="p-4 text-right">
                                                <button
                                                    onClick={() => handleEditRole(user)}
                                                    className="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors flex items-center justify-center ml-auto"
                                                    title="Ubah Tier"
                                                >
                                                    <Edit2 size={18} />
                                                </button>
